...
class g_form {
	constructor(tableName, mandatory, checkMandatory, checkNumeric, checkInteger) {
		...
		this.INFO_CLASS = "outputmsg_info";
		this.ERROR_CLASS = "outputmsg_error";
		this.INFO_ICON = "images/outputmsg_success.gifx";
		this.ERROR_ICON = "images/outputmsg_error.gifx";
		this.MSG_ROW = "_message_row";
		this.type = 'g_form';
	}
	...
	addNameMapEntry(entry) {
		this.nameMap.push(entry);
	}
	
	addGlideUIElement(/* GlideUIElement */ ed) {
	...
	registerPrefixHandler(prefix, handlerObject) {
	...
	getElement(id) {
		if (this.elementHandlers[id] &&
			(typeof this.elementHandlers[id].getElement == "function")) {
	...
	addValidator(fieldType, validator) {
	...
	_setDisplay(fieldName, display, isMandatory, fieldValue) {
		...
		if (this.elementHandlers[control.id] &&
			(typeof this.elementHandlers[control.id].getElement == "function")) {
			theElement = this.elementHandlers[control.id].getElement();
		}
		if (this.elementHandlers[control.id] &&
			(typeof this.elementHandlers[control.id].setDisplay == "function")) {
	...
	setSensitiveDisplayValue(name, displayValue) {
	...
	setVisible(fieldName, visibility) {
		fieldName = this.removeCurrentPrefix(fieldName);
		var control = this.getControl(fieldName);
		if (!control) {
			var handler = this.getPrefixHandler(fieldName);
		...
		if ((visibility != true) && this.isMandatory(fieldName) &&
	...
	changeElementStyle(fieldName, name, value) {
	...
	changeElementParent(ge, name, value) {
	...
	setReadonly(fieldName, disabled) {
		fieldName = this.removeCurrentPrefix(fieldName);
		this._setReadonly(fieldName, disabled, this.isMandatory(fieldName),
			this.getValue(fieldName));
	}
	
	_setReadonly(fieldName, disabled, isMandatory, fieldValue) {
		var s = this.tableName + '.' + fieldName;
		var control = this.getControl(fieldName);
		if (!control) {
			var handler = this.getPrefixHandler(fieldName);
			if (handler) {
				handler.getObject()
				._setReadonly(handler.getFieldName(), disabled, isMandatory, fieldValue);
			}
			return;
		}
		var ge = this.getGlideUIElement(fieldName);
	...
	_setReadonly0(ge, control, s, fieldName, disabled, isMandatory, fieldValue) {
		...
		if (this.elementHandlers[control.id] &&
			(typeof this.elementHandlers[control.id].setReadOnly == "function")) {
		...
		if (geType == "glide_duration" || geType == "glide_time") {
			this._setFieldReadOnly(ge, "ni." + this.tableName + "." + fieldName + "dur_day",
				disabled);
			this._setFieldReadOnly(ge, "ni." + this.tableName + "." + fieldName + "dur_hour",
				disabled);
			this._setFieldReadOnly(ge, "ni." + this.tableName + "." + fieldName + "dur_min",
				disabled);
			this._setFieldReadOnly(ge, "ni." + this.tableName + "." + fieldName + "dur_sec",
				disabled);
	...
	_readOnlyRadio(control, fieldName, disabled) {
	...
	_readOnlyRadioDirection(tr, fieldName, direction, disabled) {
		...
		while (tr) {
			if (tr.getAttribute("is_radio_option") == "true") { // service catalog will include this attribute on the TR
				...
				return;
			} // next TR didn't have the attribute (but we had them before) so we're done
			...
			{
				td = tr.childNodes[1];
			} // second node should be the TD that has the radio
			if (td.childNodes) {
				var radio = td.childNodes[0];
				if (!radio) {
					return;
				} // nothing there, we're done
	...
	_setFieldReadOnly(ge, s, disabled) {
	...
	_setDisabledField(control, disabled) {
		if (disabled) {
	...
	mandatoryCheck() {
		...
		if (!rc) {
			...
			theText =
				new GwtMessage().getMessage('The following mandatory fields are not filled in') +
				': ' + theText;
			try {
				alert(theText);
			} catch(e) {
	...
	isReadOnly(ge, control) {
		return !control.disabled;
	}
	
	isDisplayNone(ge, control) {
	...
	setTemplateValue(fieldName, value, displayValue) {
		fieldname = this.removeCurrentPrefix(fieldName);
		var control = this.getControl(fieldName);
		if (control) {
	...
	_setValue(fieldName, value, displayValue, updateRelated) {
		fieldName = this.removeCurrentPrefix(fieldName);
		var control = this.getControl(fieldName);
		...
		if (this.elementHandlers[control.id] &&
			(typeof this.elementHandlers[control.id].setValue == "function")) {
	...
	_setReferenceSelect(control, sel, value, displayValue) {
	...
	_getSelectedIndex(control, value, displayValue) {
		var options = control.options;
	...
	_ensureDisplayValue(fieldName, value, displayValue) {
		if (displayValue) {
			return displayValue;
		}
	...
	_getValueFromControl(control) {
		if (this.elementHandlers[control.id] &&
			(typeof this.elementHandlers[control.id].getValue == "function")) {
...
*/
	addOption(fieldName, choiceValue, choiceLabel, choiceIndex) {
		...
		if (choiceIndex == undefined) {
			choiceIndex = len;
		} // no position specified, put it at the end
		if (choiceIndex < 0 || choiceIndex > len) {
			choiceIndex = len;
		} // specified position is impossible, put it at the end
	...
	getOptionControl(fieldName, choiceValue) {
	...
	allChangedFieldsFilter(element) {
	...
	flash(widgetName, color, count) {
		...
		if (count < 4) {
			setTimeout('g_form.flash("' + widgetName + '", "' + originalColor + '", ' + count + ')',
				500);
		}
	}
	...
	findV2RelatedListName(listTableName) {
	...
	_addFormMessage(msg, type) {
		...
		newMsg.innerHTML =
			'<table cellspacing="0" cellpadding="0" border="0" class="background_transparent"><tbody><tr><td class="message_image"><img width="16" height="16" src="images/outputmsg_' +
			type + '.gifx"></td><td width="100%">' + msg +
			'</td><td id="close_messages" class="message_image close_messages"></td></tr></tbody></table>';
		...
		c.innerHTML =
			'<img src="images/x.gifx" width="13" height="12" onclick="window.hide(\'output_messages\');gel(\'output_messages\').innerHTML = \'\';"/>';
		e.style.display = "";
	...
	showFieldMsg(input, message, type, /* optional */ scrollForm) {
		...
		if (!inputElement || !message) {
			jslog(
				"ERROR: invalid field or missing message passed to g_form.showFieldMsg('" + input +
				"','" + message + "')");
			return;
		}
	 ...
	 */
	_scrollToElementTR(labelTR, msgTR, scrollForm) {
		...
		if (topOfLabel > scrollDiv.scrollTop + scrollDiv.clientHeight) {
			needToScroll = true;
		}// label is below the current display
		else if (topOfLabel < scrollDiv.scrollTop + headerHeight) {
			needToScroll = true;
		}// label is above the current display
	...
	_positionCursorAtError(elem, message) {
	...
	_setCaretPositionLineColumn(elem, lineNo, columnNo) {
		var pl = 1;
		var data = elem.value;
		var len = data.length;
		var position = 0;
		...
		if (elem.createTextRange) {
			position -= lineNo - 1;
		} // don't count new line chars in this case
		try {
			...
			if (elem.setSelectionRange) { // Firefox & Webkit
				...
				if (isSafari || isChrome) // we don't have a good way to scroll in these
				{
					...
					ev.initKeyEvent("keypress", true, true, window, false, false, false, false, 0,
						elem.value.charCodeAt(caretPos - 1));
...
var GlideFormPrefixHandler = class {
	constructor(handlerObject) {
		...
		this.fieldName = "";
		this.type = 'GlideFormPrefixHandler';
	}
